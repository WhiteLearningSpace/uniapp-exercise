<template>
	<view class="video-container">
		...
		<!-- 导航区 -->
		<scroll-view :scroll-into-view="'scroll'+navId" scroll-with-animation scroll-x enable-flex class="nav-scroll">
			<view :id="'scroll'+item.id" class="nav-item" v-for="item in videoGroupList" :key="item.id">
				<view class="nav-content" :class="{active:navId===item.id}" @tap="changeNav" :id="item.id">
					{{item.name}}
				</view>
			</view>
		</scroll-view>
		...
		<scroll-view class="video-scroll" scroll-y refresher-enabled :refresher-triggered="isTriggered"
			@refresherrefresh="handleRefresher" @scrolltolower="handleToLower">
			<view class="video-item" v-for="item in videoList" :key="item.id">
				<!-- 视频 -->
				<video :src="item.data.urlInfo.url" :poster="item.data.coverUrl" :id="item.data.vid" @play="handlePlay"
					v-if="videoId===item.data.vid" object-fit="cover" @timeupdate="handleTimeUpdate"
					@ended="handleEnded"></video>
				<image :src="item.data.coverUrl" @tap="handlePlay" :id="item.data.vid" v-else></image>
...
<script setup>
	...
	const videoGroupList = reactive([]);
	const navId = ref("");
	const videoList = reactive([]);
	const videoId = ref("");
	const videoUpdateTime = reactive([]);
	...
	async function getVideoGroupList() {
		...
		navId.value = videoGroupListData.data[0].id

		getvideoList(navId.value);
	};
	async function getvideoList(navId) {
		...
		videoList.length = 0;
		let index = 0;
		videoList.push(...videoListData.datas.map((item) => {
			item.id = index++;
			return item;
		}));
	...
	function changeNav(event) {
		navId.value = event.currentTarget.id >>> 0;
		videoList.length = 0;
		uni.showLoading({
			title: "正在加载...",
		});
	...
	function handlePlay(event) {
		...
		// vid !== vid && videoContext && videoContext.stop();
		// vid = vid;
		videoId.value = vid
		const videoContext = reactive(uni.createVideoContext(vid));
