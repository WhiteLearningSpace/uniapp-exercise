...
<script setup>
	...
	const coverTransform = ref("translateY(0)");
	const coverTransition = ref("");
	const recentPlayList = reactive([]);
	...
	if (userInfo.userId) {
		// 获取用户播放记录
		getUserRecentPlayList(userInfo.userId);
	}
	...
	function handleTouchMove(event) {
		...
		moveDistance.value = moveY.value - startY.value;
		if (moveDistance.value <= 0) {
			return;
		}
		if (moveDistance.value >= 80) {
			moveDistance.value = 80;
		}
		coverTransform.value = `translateY(${moveDistance.value}rpx)`;
	};
	// 处理滑动样式
	function handleTouchEnd() {
		coverTransform.value = `translateY(0rpx)`;
	...
	async function getUserRecentPlayList(userId) {
		...
		recentPlayList.push(...recentPlayListData.allData.splice(0, 10).map(item => {
